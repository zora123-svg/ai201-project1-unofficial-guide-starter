...
**Top-k:** 6 chunks per query. (Planned as 4. Raised during testing: relevant opinion chunks — e.g. the Midway Reddit thread at distance 0.347 — kept sitting just below generic rental boilerplate that shares query words, so a slightly larger k is needed to include them. Also added exact-duplicate chunk removal after a listing site produced identical chunks that flooded the top results, and smaller chunks (280/60) for the mixed-topic neighborhood guides so safety facts aren't diluted by adjacent restaurant lists.)
...
| 3 | What are the safest neighborhoods in St. Paul for students? | Macalester-Groveland, Highland Park, and Como — among St. Paul's safest, popular with students (sources 11, 12). (Reworded from an earlier "neighborhoods near UST that are safe" phrasing that retrieved poorly — see Failure Case Analysis.) |
